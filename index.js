...
function InjectPatternsPlugin(options) {
  console.log("InjectPatternsPlugin:", options);
  ...
  this.options.pattern = new RegExp(
    `(?:<!--|\\/\\*) ?(${selectors}):([a-z]+\\.[a-z]+)\\??([^ ]+)? ?(?:-->|\\*\\/)`,
    "i"
  );
}

InjectPatternsPlugin.prototype.apply = function(compiler) {
  const me = this;
...
module.exports = InjectPatternsPlugin;
